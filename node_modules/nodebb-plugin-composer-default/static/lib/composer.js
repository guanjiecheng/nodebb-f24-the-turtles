...
define('composer', [
	...
	$(document).on('click', '#post-privately', function () {
		// Get the current state of the button (public or private)
		...
		// Update the icon and text based on the new state
		if (newState === 'private') {
			$(this).find('i').removeClass('fa-lock-open').addClass('fa-lock');
			$(this).find('span').text('Private');
		} else {
			$(this).find('i').removeClass('fa-lock').addClass('fa-lock-open');
			$(this).find('span').text('Public');
		}
		
		console.log('Button state toggled to:', newState); // Debugging log
	});
	...
	async function post(post_uuid) {
		...
		var postAnonymous = postContainer.find('.post-anonymous')
		var postPrivately = postContainer.find('.post-privately')
		var bodyEl = postContainer.find('textarea');
		...
		if (action === 'topics.post') {
			...
			composerData = {
				...
				title: titleEl.val(),
				postAnonymous: postAnonymous? postAnonymous.prop('checked') : false,
				privatePost: postPrivately.attr('data-state') == 'public' ? false : true,
				content: bodyEl.val(),
				thumb: thumbEl.val() || '',
